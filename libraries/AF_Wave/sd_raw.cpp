...
/************************************/
uint16_t currentblockoffset = 0xFFFF;
...
uint16_t sd_raw_read_more(uint8_t* buffer, uint16_t length)
{
  ...
  if (length + currentblockoffset > 512)
    length = 512- currentblockoffset;
...
{
  for(; currentblockoffset < 512; ++currentblockoffset) {
    sd_raw_rec_byte();
  }
  /* read crc16 */
  sd_raw_rec_byte();
  ...
  /* let card some time to finish */
  sd_raw_rec_byte();
  currentblockoffset = 0xFFFF; // ready to do it again
}
